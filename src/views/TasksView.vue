...
import { cancelFeedArchive, getArchiveProgress, startFeedArchive, type ArchiveProgress } from "../utils/qzone";
import { useAuthStore } from "../stores/auth";
import { getArchiveInterval } from "../utils/appSettings";

const authStore = useAuthStore();
const { loggedIn } = storeToRefs(authStore);
const progress = ref<ArchiveProgress>({ status: "idle", pages: 0, fetched: 0, saved: 0, message: "尚未开始归档" });
...
async function refresh() { try { progress.value = await getArchiveProgress(); } catch { /* 保留当前状态 */ } }
...
async function start() {
  ...
  finally { if (progress.value.status === "limited") beginPolling(); else { clearInterval(timer); timer = undefined; } }
}
...
<template>
  <section class="surface-card task-card">
    ...
    <div class="task-stats"><div><span>已读取页数</span><strong>{{ progress.pages }}</strong></div><div><span>接口记录</span><strong>{{ progress.fetched }}</strong></div><div><span>写入记录</span><strong>{{ progress.saved }}</strong></div></div>
