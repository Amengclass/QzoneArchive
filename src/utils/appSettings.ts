...
export const DEFAULT_ARCHIVE_INTERVAL = 1000;

export function getArchiveInterval() {
  const value = Number(localStorage.getItem(ARCHIVE_INTERVAL_KEY));
  return Number.isFinite(value) ? Math.min(30000, Math.max(500, Math.round(value))) : DEFAULT_ARCHIVE_INTERVAL;
}

export function setArchiveInterval(value: number) {
  const normalized = Math.min(30000, Math.max(500, Math.round(value || DEFAULT_ARCHIVE_INTERVAL)));
